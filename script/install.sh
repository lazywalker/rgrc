...
download() {
  ...
  printf "\n" >&2
  info "This is likely due to rgrc not yet supporting your configuration."
  info "If you would like to see a build for your configuration,"
  info "please create an issue requesting a build for ${MAGENTA}${TARGET}${NO_COLOR}:"
  info "${BOLD}${UNDERLINE}https://github.com/lazywalker/rgrc/issues/new/${NO_COLOR}"
  return $rc
}
...
unpack() {
  ...
  info "bug report at https://github.com/lazywalker/rgrc/issues"
  return 1
}
...
usage() {
  printf "%s\n" \
    "install.sh [option]" \
    "" \
    "Fetch and install the latest version of rgrc, if rgrc is already" \
  ...
  printf "\t%s\n\t\t%s\n\n" \
    ...
    "-v, --version" "Install a specific version of rgrc (with or without leading 'v') [default: ${VERSION}]" \
...
install() {
  ...
  if test_writeable "${BIN_DIR}"; then
    sudo=""
    msg="Installing rgrc, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Installing rgrc as root, please wait…"
  fi
  info "$msg"
  ...
  if [ -z "${FORCE-}" ]; then
    ...
    if [ "$yn" != "y" ] && [ "$yn" != "yes" ]; then
      if [ "$yn" = "n" ] || [ "$yn" = "no" ]; then
        set_interrupt_handler
        set +e
        printf "Where would you like to install rgrc? ${GREY}(e.g., /usr/local/bin)${NO_COLOR}\n"
        read -p "> Install rgrc ${GREEN}${VERSION}${NO_COLOR} to: " path
        trap - INT
        rc=$?
        set -e
        BIN_DIR=$path
...
print_install() {
  ...
  do
    ...
    config_file="~/.${s}rc"
    config_cmd="eval \"\$(rgrc --aliases)\""

    case ${s} in
      fish )
        # shellcheck disable=SC2088
        config_file="~/.config/fish/config.fish"
        config_cmd="rgrc --aliases | source"
        ;;
    esac
...
is_build_available() {
  ...
  if [ "${good}" != "1" ]; then
    error "${arch} builds for ${platform} are not yet available for rgrc"
    printf "\n" >&2
    info "If you would like to see a build for your configuration,"
    info "please create an issue requesting a build for ${MAGENTA}${target}${NO_COLOR}:"
    info "${BOLD}${UNDERLINE}https://github.com/lazywalker/rgrc/issues/new/${NO_COLOR}"
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/lazywalker/rgrc/releases"
fi
...
if [ "${VERSION}" != "latest" ]; then
  # Allow VERSION to be supplied with or without a leading 'v'.
  # Normalize both forms so the GitHub release tag is always 'v<version>'
...
confirm "Install rgrc ${GREEN}${VERSION}${NO_COLOR} to ${BOLD}${GREEN}${BIN_DIR}${NO_COLOR}?"
check_bin_dir "${BIN_DIR}"

install "${EXT}"
completed "rgrc ${VERSION} installed"
